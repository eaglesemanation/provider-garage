...
import (
	...
	apisNamespaced "github.com/eaglesemanation/provider-garage/apis/namespaced"
	"github.com/eaglesemanation/provider-garage/config"
	"github.com/eaglesemanation/provider-garage/internal/clients"
	controllerCluster "github.com/eaglesemanation/provider-garage/internal/controller/cluster"
	controllerNamespaced "github.com/eaglesemanation/provider-garage/internal/controller/namespaced"
	"github.com/eaglesemanation/provider-garage/internal/features"
	"github.com/eaglesemanation/provider-garage/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Garage").DefaultEnvars()
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Garage APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Garage APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Garage controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Garage controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Garage controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Garage controllers")
	}
